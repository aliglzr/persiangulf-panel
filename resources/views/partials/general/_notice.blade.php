    ...
    <div class="d-flex flex-stack flex-grow-1 {{ util()->putIf($button, 'flex-wrap flex-md-nowrap') }}">
        @if ($title || $body)
            ...
            <div class="{{ util()->putIf($button, 'mb-3 mb-md-0') }} fw-bold">
                @if ($title)
                    <h4 class="text-{{$color}} fw-bolder">{{ $title }}</h4>
                @endif

                @if ($body)
                    <div class="fs-6 text-{{$color}} @if ($button) {{ 'pe-7' }} @endif">{!! $body !!}</div>
                @endif
            </div>
            <!--end::Content-->
        @endif
        ...
        @if($editMode)
            <livewire:system.notices.edit key="{{\Illuminate\Support\Str::random(12)}}" notice_id="{{$id}}" />
            <livewire:system.notices.delete key="{{\Illuminate\Support\Str::random(12)}}" notice_id="{{$id}}" />
    @endif
    <!--end::Wrapper-->
</div>
<!--end::Notice-->
