...
class SocialiteLoginController extends Controller
{
    ...
    public function googleCallback()
    {
        try {
            ...
            $social_info = Socialite::driver('google')->stateless()->user();
        } catch (\Exception $e) {
        ...
        if ($existing_user) {
            ...
            $existing_user->setData('telegram-ad-show', "not-showed");
            $existing_user->setData('last_login', now());
            activity('ورود به حساب')->event('login')->causedBy($existing_user)->withProperties(['ip_address', request()->ip()])->log('ورود از طریق گوگل');

            return redirect()->to(Cookie::get('redirect_uri'));
        }
        return redirect()->to(route('auth', ['message' => 'emailNotFound']));
    }
