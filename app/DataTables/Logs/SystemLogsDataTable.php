...
class SystemLogsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('date', function (Collection $model) {
                return Verta::instance($model->get('date'))->persianFormat('Y-m-d H:i:s');
            })
    ...
    {
        return [
            Column::make('DT_RowIndex')->title('ردیف')->width(100)->addClass('ps-0'),
            Column::make('message')->title('پیغام'),
            Column::make('level')->title('سطح'),
            Column::make('date')->width(200)->title('تاریخ'),
            Column::make('environment')->addClass('none')->title('محیط'),
            Column::make('file_path')->title(__('آدرس فایل لاگ'))->addClass('none'),
            Column::make('context')->addClass('none')->title('محتوا'),
        ];
    }
